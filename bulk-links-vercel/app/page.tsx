...
export default function HomePage() {
  ...
  const [useGeneratedSlug, setUseGeneratedSlug] = useState(true);
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <Label htmlFor="use-generated-slug">Use generated form for custom URL slug</Label>
                      <button
                        type="button"
                    ...
                    <p className="text-xs text-gray-500">
                      {useGeneratedSlug 
                        ? (selectedList.length > 0 
                          ? `URL slug will be: ${campaign || 'Campaign'}${selectedList.length > 0 ? `-${selectedList[0]}` : ''}${date ? `-${date}` : ''}${selectedList.length > 1 ? ' (and others)' : ''}` 
                          : `URL slug will be: ${campaign || 'Campaign'}${date ? `-${date}` : ''}`)
                        : "URL slug will be auto-generated by server (e.g., abc123)"
                      }
                    </p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
          ...
          <div className="md:col-span-2">
            ...
            <Input value={
              useGeneratedSlug 
                ? (selectedList.length > 0 
                  ? `${domain || "adtracking.link"}/${campaign || 'Campaign'}${selectedList.length > 0 ? `-${selectedList[0]}` : ''}${date ? `-${date}` : ''}`
            ...
            {selectedList.length > 1 && useGeneratedSlug && (
              <p className="text-xs text-blue-600 mt-1">Showing preview for first publication - {selectedList.length - 1} additional link(s) will be created</p>
            )}
            {!useGeneratedSlug && (
              <p className="text-xs text-blue-600 mt-1">Random short URL will be generated by server</p>
            )}
          </div>
        </div>
